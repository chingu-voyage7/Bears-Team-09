...
resource "null_resource" "vm" {
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            "sudo docker run -d -e NODE_ENV='${var.node_env}' -e API_HOST='${aws_instance.webapp.public_dns}' -e API_PORT='8000' -p 80:3000 --restart always --name frontend '${var.frontend_image}:${var.deploy_tag}'"
        ]
    }
}
