...
resource "null_resource" "restart_apps" {
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            "sudo docker run --name gateway -d --network ${var.project_name}-net --restart unless-stopped -v /tmp/nginx.conf:/etc/nginx/nginx.conf -v /etc/nginx/certs/certificate.pem:/etc/nginx/certificate.pem -v /etc/nginx/certs/private_key.pem:/etc/nginx/private_key.pem -p 80:80 -p 443:443 nginx:stable-alpine",
